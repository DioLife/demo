#define kLeaderboardID @"DioLeaderBoardID"
#define kAchievementID @"DioAchievementBoardID"
#import "YQGameCenterTool.h"

#import "MyViewController.h"
#import "MyGameScene.h"
#import "MyHomeScene.h"
#import "MyCounterHandler.h"
#import "MyPlayerNode.h"
#import "MyButtonNode.h"
#import "MyGuideNode.h"
#import "SSKeychain.h"

static const uint32_t rainCategory     =  0x1 << 0;
static const uint32_t koalaCategory    =  0x1 << 1;
@interface MyGameScene()  <SKPhysicsContactDelegate>
...
{
    MyCounterHandler * _counter;
    NSArray        * _waterDroppingFrames;
    MyPlayerNode     * _player;
...
-(id)initWithSize:(CGSize)size {
    ...
    if (self) {
        ...
        MyPlayerNode * player = [[MyPlayerNode alloc] initWithDefaultTexture:koalaTexture andAnimateTextures:_koalaAnimateTextures];
        ...
        MyGuideNode * guide = [[MyGuideNode alloc] initWithTitleTexture:[_atlas textureNamed:@"text-swipe"]
...
-(void) gameStart {
    ...
    [self addChild:score];
    _score = score;
    MyCounterHandler *counter = [[MyCounterHandler alloc] init];
...
-(void) gameOver {
    ...
    SKSpriteNode * newRecord = [SKSpriteNode spriteNodeWithColor:[SKColor clearColor] size:CGSizeMake(0.0, 0.0)];
    ...
    MyCounterHandler * currentScore = [[MyCounterHandler alloc] initWithNumber:[_counter getNumber]];
    currentScore.position = CGPointMake(CGRectGetMidX(self.frame) + 105, CGRectGetMidY(self.frame) - 4.0);
    MyCounterHandler * highScore = [[MyCounterHandler alloc] initWithNumber:[self getHighScore]];
    ...
    MyButtonNode * homeButton = [[MyButtonNode alloc] initWithDefaultTexture:homeDefault andTouchedTexture:homeTouched];
    ...
    [homeButton setMethod: ^ (void) {
        ...
        SKScene * homeScene = [[MyHomeScene alloc] initWithSize:self.size];
    ...
    MyButtonNode * shareButton = [[MyButtonNode alloc] initWithDefaultTexture:shareDefault andTouchedTexture:shareTouched];
    ...
    [shareButton setMethod: ^ (void) {
        MyViewController * viewController = (MyViewController *) self.view.window.rootViewController;
        NSString * sharetext = [NSString stringWithFormat:@"I just scored %d in #Agile Koala!", (int) [self->_counter getNumber]];
        ...
        NSString * score = [NSString stringWithFormat:@"%d", (int)[self->_counter getNumber]];
    ...
    MyButtonNode * rateButton = [[MyButtonNode alloc] initWithDefaultTexture:rateDefault andTouchedTexture:rateTouched];
    ...
    [rateButton setMethod: ^ (void) {
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:
                                                    @"itms-apps://itunes.apple.com/app/id1470900592"
                                                    ]];
    } ];
    SKTexture * retryDefault = [_atlas textureNamed:@"button-retry-off"];
    SKTexture * retryTouched = [_atlas textureNamed:@"button-retry-on"];
    MyButtonNode * retryButton = [[MyButtonNode alloc] initWithDefaultTexture:retryDefault andTouchedTexture:retryTouched];
    ...
    [retryButton setMethod: ^ (void) {
        ...
        SKScene * gameScene = [[MyGameScene alloc] initWithSize:self.size];
    ...
    //把每一局分数上传到gamecenter
    if ([_counter getNumber] > 10000) {
        NSLog(@"你的数据有问题");
    }
    [[YQGameCenterTool defaultTool] setRaitScoreWithSocre:[NSNumber numberWithLong:[_counter getNumber]]
...
-(void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    [MyButtonNode doButtonsActionEnded:self touches:touches withEvent:event];
...
-(void) touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    [MyButtonNode doButtonsActionBegan:self touches:touches withEvent:event];
