...
SportHeaderDelegate,SportGameItemDelegate,SportWindowDelegate {
    ...
    let footballCategoryDic = [["MN":"全部"],["TI":"波胆"],["BC":"总入球"],["HF":"半场/全场"],["MX":"混合过关"],["br":"下注记录"],["rule":"体育规则"]]
    let basketballCategorys = [["MN":"全部"],["MX":"混合过关"],["br":"下注记录"],["rule":"体育规则"]]
    ...
    override func viewDidLoad() {
        ...
        pageButton.addTarget(self, action: #selector(onPageButton), for: .touchUpInside)
...
//        self.customRightView()
    ...
    func onMenuItemClick(index:Int,dic:[Dictionary<String,String>]) -> Void {
        if index == dic.count - 2{
//            openTouzhuRecord(controller: self, title: "体育投注记录", code: String.init(describing: currentSegmentPage), recordType: MenuType.SPORT_RECORD)
            
            if let config = getSystemConfigFromJson(){
                if config.content != nil{
                    let new_sport = config.content.new_onoff_sports_game
                    if new_sport == "on"{
                        openTouzhuRecord(controller: self, title: "新体育投注记录", code: String.init(describing: self.currentSegmentPage), recordType: MenuType.NEW_SPORT_RECORD)
                    }else{
                        openTouzhuRecord(controller: self, title: "旧体育投注记录", code: String.init(describing: self.currentSegmentPage), recordType: MenuType.SPORT_RECORD)
                    }
                }
            }
            ...
            //体育规则
            let url = String.init(format: "%@%@%@", BASE_URL,PORT,"/help/QA_sport.do")
...
//        countDownUI.text = "距离同步赛事还有60秒"
    ...
    func onSegmentChange(sender:UISegmentedControl) -> Void {
    ...
    //点击刷新
    func onRefreshClick() -> Void {
        pageIndex = 1
        loadSportDatas(pageIndex: pageIndex, segmentPage: currentSegmentPage, currentPage: currentPage, categoryType: getCurrentCategory(), showDialog: true)
    }
    
    func validOdd(data:SportBetData){
                    ...
                    callback: {(resultJson:String,resultStatus:Bool)->Void in
                        ...
                        if let result = SportValidBetWraper.deserialize(from: resultJson){
                            if result.success{
                                ...
                                if validResult.count == self.selectSports.count{
                                    for index in 0...validResult.count-1{
                                        let sport:SportBean = self.selectSports[index]
                                        sport.gid = String.init(describing: validResult[index].gid)
                                        sport.peilv = String.init(describing: validResult[index].odds)
                                    }
                                }
//                                let validOdd = validResult[0]
    ...
    func formSportBetJson(ballType:Int,gameCategory:String,playCategory:String,selectSports:[SportBean],
                          acceptBestOdds:Bool,money:String,confirmGid:Int32,odd:Float) ->  SportBetData{
...
//            }
    ...
    //点击查看体育寒事注单
    func onOrderClick() -> Void {
        showOrderWindow()
    }
    
    func onPageButton(sender:UIButton) -> Void{
//        loadSportDatas(pageIndex: pageIndex, segmentPage: currentSegmentPage, currentPage: currentPage, categoryType: getCurrentCategory(), showDialog: true)
        show_select_page_dialog(sender: sender)
    }
    
    func updatePageButton(){
        if pageCount == 0{
            self.pageButton.setTitle("1/1", for: .normal)
        }else{
            let name = String.init(format: "%d/%d", self.pageIndex,self.pageCount)
            self.pageButton.setTitle(name, for: .normal)
        }
    }
    
    func show_select_page_dialog(sender:UIButton) -> Void {
        if self.pageCount == 0{
            return
        }
        let alert = UIAlertController.init(title: "页码选择", message: nil,
                                           preferredStyle: .actionSheet)
        for index in 1...self.pageCount{
            let action = UIAlertAction.init(title: String(index), style: .default, handler: {(action:UIAlertAction) in
                self.pageIndex = index
                self.loadSportDatas(pageIndex: self.pageIndex, segmentPage: self.currentSegmentPage, currentPage: self.currentPage, categoryType: self.getCurrentCategory(), showDialog: true)
            })
...
//            header.leagueUI.text = String.init(format: "%@(%d)场", league!,data.count)
            header.leagueUI.text = String.init(format: "%@", league!)
        }
