...
//    var sign_icon:UIImageView!
    ...
    init(icon:UIImage?,account:String?,balance:Float?,slideMenu:Bool,controller:BaseController) {
        ...
        bgImg = UIImageView.init(frame: CGRect.init(x: 0, y: 0, width: self.bounds.width, height: self.bounds.height))
        bgImg.backgroundColor = UIColor(hex: 0xEC2829)
//        bgImg.image = UIImage.init(named: "header_bg")
        updateHeaderBgLogo()
        self.addSubview(bgImg)
        
        app_icon = UIImageView.init(frame: CGRect.init(x: self.bounds.width*0.5-30, y: self.bounds.height*0.5-40-25, width: 60, height: 60))
        app_icon.image = icon
        app_icon.tag = 10
...
//        self.addSubview(sign_icon)
        ...
        if let config = getSystemConfigFromJson(){
            if config.content != nil{
                let txt = config.content.onoff_sign_in
                let viewWidth = slideMenu ? kScreenWidth/4 : kScreenWidth/2
                let signbtn_x = !slideMenu ? kScreenWidth*0.75-kScreenWidth/4+10 : (kScreenWidth*0.75-kScreenWidth/4+10)/2+10
                if txt == "on"{
                    accountLabel = UILabel.init(frame: CGRect.init(x: 10, y: self.bounds.height-112, width: viewWidth, height: 25))
                    ...
                    signBtn = UIButton.init(frame: CGRect.init(x: signbtn_x, y: self.bounds.height-112, width: 60, height: 22))
                    ...
                    accountLabel = UILabel.init(frame: CGRect.init(x: 0, y: self.bounds.height-112, width: self.bounds.width, height: 25))
                    accountLabel.text = account
                    accountLabel.textColor = UIColor.white
                    accountLabel.font = UIFont(name: "HelveticaNeue-Medium", size: 16)
                    ...
                    signBtn = UIButton.init(frame: CGRect.init(x: kScreenWidth*0.75-kScreenWidth/4+10, y: self.bounds.height-112, width: 60, height: 22))
//                    signBtn.setImage(UIImage.init(named: "sign_btn"), for: .normal)
//                    signBtn.addTarget(self, action: #selector(onSignClick), for: .touchUpInside)
//                    self.addSubview(signBtn)
                }
...
//
        let moneyViewWidth = slideMenu ? kScreenWidth/2 : kScreenWidth
        balanceLabel = UILabel.init(frame: CGRect.init(x: 0, y: self.bounds.height-85, width: moneyViewWidth, height: 25))
    ...
    func updateAppLogo() -> Void {
        ...
        var logoImg = sys.content.member_center_logo_url
        if !isEmptyString(str: logoImg){
            ...
            self.app_icon.image = UIImage.init(named: "charge_default_header")
        }
    }
    
    func updateHeaderBgLogo() -> Void {
        guard let sys = getSystemConfigFromJson() else{return}
        var logoImg = sys.content.member_center_bg_url
        if !isEmptyString(str: logoImg){
            //这里的logo地址有可能是相对地址
            print("the logo url = ",logoImg)
            if logoImg.contains("\t"){
            ...
            downloadImage(url: URL.init(string: logoImg)!, imageUI: self.bgImg)
        }else{
            self.bgImg.backgroundColor = UIColor.init(hex: 0xEC2829)
        }
    }
...
//        qr_icon.addGestureRecognizer(tap2)
    ...
    func openUserCenter() -> Void {
        if controller.navigationController != nil{
            YiboGameIos.openUserCenter(controller: controller)
        }else{
    ...
    func actionChargeMoney(){
        ...
        if YiboPreference.getAccountMode() == ACCOUNT_PLATFORM_TEST_GUEST{
            showToast(view: controller.view, txt: "对不起，试玩账号无法进行充值")
            return
        }
    ...
    func actionPickMoney(){
        if YiboPreference.getAccountMode() == ACCOUNT_PLATFORM_TEST_GUEST{
            showToast(view: controller.view, txt: "对不起，试玩账号无法进行提款")
            return
        }
        if controller.navigationController != nil{
...
//        toggleBtns()
    ...
    func tapClickEvent(_ recongnizer: UIPanGestureRecognizer) {
