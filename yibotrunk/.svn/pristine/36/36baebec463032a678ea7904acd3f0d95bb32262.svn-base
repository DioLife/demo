...
class BannerView: UIView {
    ...
    open var item: BannerItem? {
        didSet {
            ...
            if !isEmptyString(str: (item?.imageUrl)!){
                if let imageUrl = item?.imageUrl{
                    if let imageURL = URL(string: imageUrl){
                        icon.kf.setImage(with: ImageResource(downloadURL: imageURL), for: .normal, placeholder: UIImage(named: "charge_default_header"), options: nil, progressBlock: nil, completionHandler: nil)
                    }
                }
            }
        }
