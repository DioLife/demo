...
class TueijianAddViewController: BaseController,UITextFieldDelegate {
...
//    @IBOutlet weak var sureBtnbottom: NSLayoutConstraint!
    ...
    @IBAction func action01(_ sender: UIButton) {
        ...
        if isEmptyString(str: myUserName.text!){
            showToast(view: self.view, txt: convertString(string: "请输入用户名"))
        }
        if isEmptyString(str: passwords.text!) || isEmptyString(str: surepassword.text!){
            showToast(view: self.view, txt: convertString(string: "请输入密码"))
        }
        ...
        request(frontDialog: true, method: .post, loadTextStr: "正在添加...", url:RECOMMAND_ADDUSER, params: ["username":self.myUserName.text!,"pwd":self.passwords.text!,"okPwd":self.surepassword.text!] ) { (resultJson:String, resultStatus:Bool) in
