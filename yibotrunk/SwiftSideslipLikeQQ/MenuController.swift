...
class MenuController: BaseMainController, UITableViewDelegate, UITableViewDataSource {
    ...
    func adjustLeftMenusBySystemConfig() -> Void {
        ...
        if let sys = sysConfig{
            ...
            //调整第三组项
            //是否显示代理管理
            var arr2 = self.datas[2]["menus"] as! [Dictionary<String,String>]
            var newArr2:[Dictionary<String,String>] = []
            for index in 0...arr2.count-1{
                let dic = arr2[index] as Dictionary<String,String>
                if dic["itemName"] == "用户列表"{
//                    if !isEmptyString(str: sys.content.onoff_all_level_fixed) && sys.content.onoff_all_level_fixed == "on"{
                        newArr2.append(dic)
//                    }
                }
                if dic["itemName"] == "我的推荐"{
                    if !isEmptyString(str: sys.content.onoff_all_level_fixed) && sys.content.onoff_all_level_fixed == "on"{
                        newArr2.append(dic)
                    }
                }
