//
//  QueryResultsWeiZhongJiangController.swift
//  gameplay
//
...
class QueryResultsWeiZhongJiangController: QueryResultsBasicController {
    ...
    func loadNetRequest(isLoadMoreData: Bool,pageSize: Int = defaultCountOfRequestPagesize) {
        ...
        LennyNetworkRequest.obtainLotteryQuery(lotteryCode: self.filterLotCode, startTime: self.filterStartTime, endTime: self.filterEndTime, qihao: "", version: "", order: "", zuihaoOrder: "", status: "3", queryType: "1", playCode: "", include: "1", username: self.filterUsername,pageSize: "\(pageSize)",pageNumber: "\(pageNumber)") { (model) in
