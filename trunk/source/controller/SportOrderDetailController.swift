...
//        cell.valueUI.text = datas[indexPath.row].value
        do{
            let attrStr = try NSAttributedString.init(data: datas[indexPath.row].value.description.data(using: String.Encoding.unicode)!, options: [.documentType:NSAttributedString.DocumentType.html], documentAttributes: nil)
    ...
    func toBetJson(detail:SportOrderDetail,result:CampionResult) -> String {
        ...
        var home = isEmptyString(str: result.firstTeam) ? detail.homeTeam : result.firstTeam
        var guest = isEmptyString(str: result.lastTeam) ? detail.guestTeam : result.lastTeam
