...
//    var userType = "" //用户类型
    ...
    @IBAction func createBtn(_ sender: UIButton) {
        ...
        if isEmptyString(str: self.sportRebate) && sport == "on"{
            showToast(view: self.view, txt: "请设置体育返点!!!")
            return
        }
        
        if isEmptyString(str: self.shabaRebate) && sbsport == "on"{
            showToast(view: self.view, txt: "请设置沙巴体育返点!!!")
            return
        }
        
        if isEmptyString(str: self.realRebate) && zhenren == "on"{
            showToast(view: self.view, txt: "请设置真人返点!!!")
            return
        }
        
        if isEmptyString(str: self.eqameRebate) && game == "on"{
