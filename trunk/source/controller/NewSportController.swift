...
protocol NewSportHeaderDelegate {
    func onHeaderClick(section:Int)
}

protocol NewSportGameItemDelegate {
    func onGameItemClick(sport:NewSportBean)
}

//体育页面
class NewSportController: BaseController,UITableViewDelegate,UITableViewDataSource,SportMenuBarDelegate,
NewSportHeaderDelegate,NewSportGameItemDelegate,NewSportWindowDelegate {
    ...
    let footballCategoryDic = [["MN":"全部"],["TI":"波胆"],["BC":"总入球"],["HF":"半场/全场"],["MX":"混合过关"],["br":"下注记录"],
                               ["gz":"体育规则"]]
    let basketballCategorys = [["MN":"全部"],["MX":"混合过关"],["br":"下注记录"],["gz":"体育规则"]]
    ...
    var sportWindow:NewSportPopWindow!
    ...
    override func viewDidLoad() {
        ...
        setThemeViewThemeColor(view: menuLine)
...
//        self.customRightView()
    ...
    func onMenuItemClick(index:Int,dic:[Dictionary<String,String>]) -> Void {
        if index == dic.count - 1{
            let url = String.init(format: "%@%@%@", BASE_URL,PORT,"/sport/hgHelp/generalRule.do")
            openActiveDetail(controller: self, title: "体育规则", content: "", foreighUrl: url)
        }else if index == dic.count - 2{
            openTouzhuRecord(controller: self, title: "体育投注记录", code: String.init(describing: currentSegmentPage), recordType: MenuType.SPORT_RECORD)
        }else{
...
//        countDownUI.text = "距离同步赛事还有60秒"
        ...
        setThemeLabelTextColorWithNavTextColor(label: countDownUI)
    ...
    func validOdd(data:NewSportBetData){
        let postJson = data.toJSONString()
        if let json = postJson{
            print("the sport post json = \(json)")
            request(frontDialog: true,method: .post,loadTextStr: "正在校验...",url:SPORT_VALID_BETS_URL,params:["data":json],
                    callback: {(resultJson:String,resultStatus:Bool)->Void in
                        if !resultStatus {
                            if resultJson.isEmpty {
                                showToast(view: self.view, txt: convertString(string: "校验失败"))
                            }else{
                                showToast(view: self.view, txt: resultJson)
                            }
                            return
                        }
                        if let result = SportValidBetWraper.deserialize(from: resultJson){
                            if result.success{
                                
                                UserDefaults.standard.setValue(result.accessToken, forKey: "token")
                                guard let content = result.content else{
                                    showToast(view: self.view, txt: "下注失败")
                                    return
                                }
//                                guard let validResult = content.odds else{return}
                                if content.isEmpty{
                                   return
                                }
                                let validOdd = content[0]
                                let data = self.formSportBetJson(ballType: self.currentSegmentPage, gameCategory: self.GAME_TYPES[self.currentPage], playCategory: self.getCurrentCategory()
                                    , selectSports: self.selectSports, acceptBestOdds: self.appcetBestPeilv, money: self.inputMoney,confirmGid: validOdd.gid,odd: validOdd.odds)
                                
                                let postJson = data.toJSONString()
                                if let json = postJson{
                                    print("the sport post json = \(json)")
                                    self.request(frontDialog: true,method: .post,loadTextStr: "正在下注...",url:SPORT_BETS_URL,params:["data":json],
                                            callback: {(resultJson:String,resultStatus:Bool)->Void in
                                                if !resultStatus {
                                                    if resultJson.isEmpty {
    ...
    func showBetSuccessDialog() -> Void {
        let alertController = UIAlertController(title: "温馨提示",
                                                message: "下注成功！是否查看注单？", preferredStyle: .alert)
        let viewAction = UIAlertAction(title: "查看记录", style: .cancel, handler: {
            action in
            openTouzhuRecord(controller: self, title: "体育投注记录", code: String.init(describing: self.currentSegmentPage), recordType: MenuType.SPORT_RECORD)
        })
    ...
    @objc func onTouzhuClick() -> Void {
        ...
        let data = self.formSportBetJson(ballType: self.currentSegmentPage, gameCategory: self.GAME_TYPES[self.currentPage], playCategory: self.getCurrentCategory()
            , selectSports: self.selectSports, acceptBestOdds: self.appcetBestPeilv, money: self.inputMoney,confirmGid: 0,odd: 0)
        validOdd(data: data)
     ...
     * @param money 下注金额
     */
    func formSportBetJson(ballType:Int,gameCategory:String,playCategory:String,selectSports:[NewSportBean],
                          acceptBestOdds:Bool,money:String,confirmGid:Int64,odd:Float) ->  NewSportBetData{
        let sportBetData = NewSportBetData()
        ...
        var items = [NewSportBet]()
        for sport in selectSports{
            let sportBet = NewSportBet()
            if confirmGid > 0{
                sportBet.gid = String.init(describing: confirmGid)
            }else{
                sportBet.gid = sport.gid
            }
            ...
            if odd > 0{
                sportBet.odds = String.init(describing: odd)
            }else{
                sportBet.odds = peilv
            }
            //足球滚球时将主客队比分传到后台
            if gameCategory == "RB" && BALL_TYPES[ballType] == "FT"{
                sportBet.scoreH = sport.scoreH
                sportBet.scoreC = sport.scoreC
            }
...
//            }
    ...
    func showOrderWindow() -> Void {
        ...
        if sportWindow == nil{
            sportWindow = Bundle.main.loadNibNamed("new_sport_pop_window", owner: nil, options: nil)?.first as! NewSportPopWindow
        }
    ...
    func loadSportDatas(pageIndex:Int,segmentPage:Int,currentPage:Int,categoryType:String,showDialog:Bool) -> Void {
        ...
        request(frontDialog: showDialog, method: .post, loadTextStr: "获取赛事中...", url:SPORT_DATA_URL,params:params,
                callback: {(resultJson:String,resultStatus:Bool)->Void in
                    if !resultStatus {
                        if resultJson.isEmpty {
    ...
    @objc func tickGameTimer() -> Void {
        ...
        if self.endGameTime <= 0{
            ...
            //再次获取赛事
            loadSportDatas(pageIndex: pageIndex, segmentPage: currentSegmentPage, currentPage: currentPage, categoryType: getCurrentCategory(), showDialog: false)
        }
    }
...
//        inputMoney = ""
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "gameCell") as? NewGameCell  else {
            ...
            if currentSegmentPage == FOOTBALL_PAGE{
                ...
                if let retime = retimeset{
                    if !isEmptyString(str: retime) && retime.contains("^"){
                        ...
                        let timeValue = (retime as NSString).substring(from: halfValue.count+1)
...
//                        print("halfvalue = \(halfValue),timeValue = \(timeValue)")
        ...
        //绑定表格头部行
        let columns = cell.tableHeader.setupView(arr: NewSportTableContainer.getTableHeaderArray(ballType: self.currentSegmentPage, playCategory: self.getCurrentCategory()))
        
        //根据赛事数据计算显示赛事结果的数据
        let sportBeans = NewSportTableContainer.fillSportResultData(item: header as Dictionary<String, AnyObject>, ballType: currentSegmentPage, playCategory: self.getCurrentCategory(), columns: columns, gameCategory: GAME_TYPES[currentPage])
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        //绑定表格头部行
        let columns = NewSportTableContainer.getTableHeaderArray(ballType: self.currentSegmentPage, playCategory: self.getCurrentCategory()).count
        let data:[Dictionary<String,String>] = self.sportDatas[indexPath.section]
        let header:Dictionary<String,String> = data[indexPath.row]
        //根据赛事数据计算显示赛事结果的数据
        let sportBeans = NewSportTableContainer.fillSportResultData(item: header as Dictionary<String, AnyObject>, ballType: currentSegmentPage, playCategory: self.getCurrentCategory(), columns: columns, gameCategory: GAME_TYPES[currentPage])
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        let header = NewSportHeader.init(frame: CGRect.init(x: 0, y: 0, width: tableView.bounds.width, height: 41))
...
//        header.backgroundColor = isExpand ? UIColor.init(red: 252/255, green: 248/255, blue: 208/255, alpha: 1.0) : UIColor.white
        ...
        setupNoPictureAlphaBgView(view: header)
    ...
    //点击投注单弹窗中的投注按钮的回调
    func onSportWindowCallback(acceptBestPeilv:Bool,inputMoney:String,notAutoPop:Bool,datas:[NewSportBean]) {
    ...
    //点击注单窗口中的暂不投注回调
    func onSportWindowClose(acceptBestPeilv: Bool, inputMoney: String,notAutoPop:Bool,datas:[NewSportBean]) {
        self.appcetBestPeilv = acceptBestPeilv
        self.inputMoney = inputMoney
        self.noAutoPopWindow = notAutoPop
        self.selectSports.removeAll()
