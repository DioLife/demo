...
import UIKit
//每场赛事子单元表格cell
class NewGameCell: UITableViewCell,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    var gameDatas:[NewSportBean] = []
    var sportItemDelegate:NewSportGameItemDelegate?
    var columns:Int = 0
...
//        tableHeader.layer.mask = headerLayer
    ...
    func setupDataAndReload(datas:[NewSportBean],columns:Int) -> Void {
...
//        //设置纵向间距-行间距
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "gameCollectionCell", for: indexPath) as! NewGameCollectionCell
        let gameData = self.gameDatas[indexPath.row]
        if gameData.isHeader{
