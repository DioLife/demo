 ...
 public class LTSimpleManager: UIView {
    ...
    @objc public func configHeaderView(_ handle: (() -> UIView?)?) {
        ...
        setViewBackgroundColorTransparent(view: headerView)
    ...
    @objc public init(frame: CGRect, viewControllers: [UIViewController], titles: [String], currentViewController:UIViewController, layout: LTLayout) {
        ...
        setViewBackgroundColorTransparent(view: pageView)
        createSubViews()
        
        self.header = LZDHeader(frame: CGRect(x: 0, y: 0, width: self.width, height: 120))
        setViewBackgroundColorTransparent(view: self.header)
        tableView.gtm_addRefreshHeaderView(refreshHeader: self.header, refreshBlock: {
            playAlertSound(path: "pulldownsome", isShock: false)
...
//            for item in allData {
//                print(item.userName!,"\t\t\tcpName:",item.cpName!,"\t\t\tczCode:",
//                      item.czCode!,"\tago:",item.ago!,"\tcpBianHao:",item.cpBianHao!,
//                      "\tlotTyp:",item.lotType!,"\tlotVersion:",item.lotVersion!,"\t\tnum:",
//                      item.num!,"\t\t\ticon:",item.icon!)
//            }
        })
    }
