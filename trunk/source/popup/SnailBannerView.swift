...
open class BannerItem: NSObject {
    ...
    public var image: String?, title: String?
    
    static func item(title: String, image: String) -> BannerItem {
        let item = BannerItem()
        item.title = title
        item.image = image
        return item
    ...
    open var item: BannerItem? {
        didSet {
            if let itemValue = item {
                if let itemImage = itemValue.image {
                    icon.theme_setImage(ThemeImagePicker.init(keyPath: itemImage), forState: .normal)
                }
            }
