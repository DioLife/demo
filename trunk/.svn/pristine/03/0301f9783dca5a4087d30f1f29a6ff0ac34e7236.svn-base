...
import UIKit
//彩票，六合彩投注记录详情
class TouzhuRecordDetailController: BaseTableViewController,UITableViewDelegate,UITableViewDataSource {
                    ...
                    if let result = LotteryRecordDetailWraper.deserialize(from: resultJson){
                        if result.success{
                            ...
                            if let content = result.content{
                                ...
                                dic2["name"] = "单注金额:"
                                dic2["value"] = true ? String.init(format: "%.2f元", content.buyMoney/Float(content.buyZhuShu)) : String.init(format: "%.2f元", (2.0/Float(content.model)))
                                ...
                                let isPeilvVersion = isSixMark(lotCode: self.cpBianma) ? true : false
