...
//彩种限额，彩种信息
class LotInfoListController: BaseController {
...
//        setupNoPictureAlphaBgView(view: viewHeader, alpha: 0.4)
        setThemeViewNoTransparentDefaultGlassBlackOtherGray(view: viewHeader)
...
extension LotInfoListController {
...
extension LotInfoListController : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if self.currentLimitInfo != nil{
            if let playInfos = self.currentLimitInfo.playInfo{
                cell.setModel(play: playInfos[indexPath.row])
            }
        }
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if self.currentLimitInfo != nil{
            if let playInfos = self.currentLimitInfo.playInfo{
                let rowNum = playInfos[indexPath.row].rowNum
                return CGFloat(44*rowNum)
            }
        }
        return 44
    }
