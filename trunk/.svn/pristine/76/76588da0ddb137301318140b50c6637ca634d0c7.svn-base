...
protocol NewSportWindowDelegate {
...
class NewSportPopWindow: UIView,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate{
    ...
    var orderDatas:[NewSportBean] = []
    var windowDelegate:NewSportWindowDelegate?
    var keyBoardNeedLayout: Bool = true
    
    var minBetAccount:Float = 10//单注最低10元
    ...
    override func awakeFromNib() {
        ...
        let nib = UINib(nibName: "new_sport_order_cell", bundle: nil) //nibName指的是我们创建的Cell文件名
    ...
    @objc func onTouzhuClick() -> Void {
        ...
        let inputTxt = inputMoneyUI.text!
        if !isEmptyString(str: inputTxt){
            if Float(inputTxt)! < self.minBetAccount{
                showToast(view: self._window, txt: String.init(format: "最低下注金额%.1f元", self.minBetAccount))
                return
            }
        }
...
//        }
    ...
    func setData(items:[NewSportBean],acceptPeilv:Bool,inputMoney:String,notAutoPop:Bool){
        ...
        minMoneyUI.text = String.init(format: "单注最低:%.1f元", self.minBetAccount)
        inputMoneyUI.text = inputMoney
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if !isEmptyString(str: data.betTeamName){
            cell.projectAndPeilv.text = String.init(format: "%@ @ %@", data.betTeamName,peilv)
        }else{
            cell.projectAndPeilv.text = String.init(format: "%@ @ %@", data.betTeamName,peilv)
        }
