...
- (void)setupSubView{
//    UIImageView *bgV = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"bg"]];
...
- (CGPoint)getRefPoint{
    ...
    if (absoluteRect.origin.y + absoluteRect.size.height +  menuHeight > ScreenHeight - 10) {
...
- (void)calculateArrowAndFrame{
    ...
    // 考虑向左右展示不全的情况，需要反向展示
    if (originX + width > ScreenWidth - 10) {
        originX = ScreenWidth - kDefaultMargin - width;
    }else if (originX < 10) {
    ...
    //如果不是根据关联视图，得算一次是否反向
    if (!_refView) {
        _isReverse = (originY + height > ScreenHeight - kDefaultMargin)?YES:NO;
    }
